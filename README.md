...
`go-webhook` is a protocol-independent Go module for exact-byte webhook
...
Go 1.26 or newer is required because the optional published `go-outbox`
adapter requires it.
...
Optional packages integrate `go-idempotency`, `go-log`, `go-telemetry`,
`go-queue`, and `go-outbox`. `webhooktest` supplies deterministic consumer
fixtures.
...
- [Replay and go-idempotency](docs/replay.md)
