#!/usr/bin/env python3
"""Independently generate and verify go-webhook v1 golden vectors."""
...
def canonical(vector: dict[str, object]) -> bytes:
    ...
    query = urlencode(
        [(key, value) for key in sorted(query_values) for value in sorted(query_values[key])]
    )
    metadata = "\n".join(
    ...
    lines = [
        ...
        f"method:{str(vector['method']).upper()}",
