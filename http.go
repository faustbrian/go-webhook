...
func formatSignatureHeader(signature Signature) (string, error) {
	if signature.Version != "v1" || signature.KeyID == "" || signature.Timestamp.IsZero() || !validNonce(signature.Nonce) {
...
func (s *Signer) SignRequest(request *http.Request, options RequestOptions) ([]Signature, []byte, error) {
	...
	message := requestMessage(request, body, options.Metadata)
...
func (v *Verifier) VerifyRequest(
	...
	verification, err = v.Verify(requestMessage(request, body, options.Metadata), signatures)
	if err != nil {
...
func requestMessage(request *http.Request, body []byte, metadata map[string]string) Message {
