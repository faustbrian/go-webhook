...
`go-webhook`, then implement every justified hardening change required at an
...
- relevant HTTP signature, digest, timestamp, and webhook documentation used
  by generic schemes
- `go-http-client`, `go-outbox`, and `go-queue` contracts for optional adapters
