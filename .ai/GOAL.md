...
- compatible with `go-http-client`, `go-queue`, and `go-outbox`
...
- event ID extraction and optional `go-idempotency` replay-store integration
...
- OpenTelemetry propagation and traces through optional `go-telemetry`
  integration
- secret-safe structured diagnostics compatible with `go-log`
...
- cryptographic replay protection and `go-idempotency` integration guide
- outbound delivery, retry, dead-letter, and replay guide
- endpoint security and SSRF threat model
- integration guides for `go-http-client`, `go-queue`, and `go-outbox`
