...
func FuzzCanonicalize(f *testing.F) {
	f.Add("POST", "/hooks", "a=1", "example.com", []byte("body"), "key")
	f.Add("\n", "%", "%", "", []byte{0xff}, "")
	f.Fuzz(func(t *testing.T, method, path, query, host string, body []byte, keyID string) {
		message := Message{
			Timestamp: time.Unix(1_700_000_000, 0), Method: method, Path: path,
			RawQuery: query, Host: host, Body: body,
		}
...
func FuzzTimestampVerification(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seconds int64) {
		...
		signature := Signature{Version: "v1", Algorithm: SHA256, KeyID: "key", Timestamp: time.Unix(seconds, 0), Value: "AA"}
