...
func Canonicalize(message Message, keyID string, algorithm Algorithm) ([]byte, error) {
	...
	if strings.ContainsAny(message.Method, "\r\n") {
		return nil, fmt.Errorf("%w: method contains a line break", ErrInvalidConfiguration)
	}
...
func NewSigner(config SignerConfig) (*Signer, error) {
	...
	for index, key := range config.Keys {
		if key.ID == "" || len(key.Secret) == 0 {
			return nil, fmt.Errorf("%w: key ID and secret are required", ErrInvalidConfiguration)
		}
...
func (s *Signer) Sign(message Message) ([]Signature, error) {
	...
	for _, key := range s.keys {
		if activeKey(key.NotBefore, key.NotAfter, key.Revoked, now) {
			keys = append(keys, key)
		}
	}
	if len(keys) == 0 {
...
func (v *Verifier) Verify(message Message, signatures []Signature) (Verification, error) {
	...
	for _, signature := range signatures {
		...
		if !message.Timestamp.IsZero() && !message.Timestamp.Equal(signature.Timestamp) {
			continue
		}
...
func (v *Verifier) recordReplay(ctx context.Context, verification Verification, eventID string) (err error) {
	...
	key := replayKey(v.namespace, eventID, verification.KeyID)
...
func replayKey(namespace, eventID, keyID string) string {
	digest := sha256.New()
	for _, value := range []string{namespace, eventID, keyID} {
		var size [8]byte
